...
def mlParams(X, labels, W=None):
    assert (X.shape[0] == labels.shape[
        0])  # test that condition, and immediately trigger an error if the condition is false.
    Npts, Ndims = np.shape(X)  # Npts = N  Number of datapoints, Ndims = d  Dimension of the data
    classes = np.unique(labels)  # extracts the classes, labeled with numbers
    Nclasses = np.size(classes)  # Nclasses = C Number of classes

    if W is None:
        W = np.ones((Npts, 1)) / float(Npts)

    mu = np.zeros((Nclasses, Ndims))  # mu = C x d matrix
    sigma = np.zeros((Nclasses, Ndims, Ndims))  # sigma = C x d x d matric
...
def trainBoost(base_classifier, X, labels, T=10):
    ...
    for i_iter in range(0, T):
        ...
        res = (vote == labels)
        res = res.reshape(Npts, 1)
        eCur = np.sum(wCur * (1 - res))
        alphaCur = 0.5 * (np.log(1. - eCur) - np.log(eCur))

        wNext = wCur * np.where(res == 1, np.exp(-1 * alphaCur), np.exp(alphaCur))
